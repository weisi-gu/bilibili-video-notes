...
确认后，**由当前 agent 直接按模板写即可**（Claude Code / Codex / WorkBuddy / Cursor 等主流 AI Agent 都能胜任，这是推荐路径）。
也可用 `scripts/make_notes.py` 把逐字稿+模板组装成一份 `note_prompt.md` 草稿，整段粘给任意 AI 即可——脚本本身不调用任何 LLM。
...
- **取数**：`bilibili-api-python`（元数据 `get_info` + 音频直链 `get_download_url`，**二者**匿名可用）、`aiohttp`（下载音频）、`yt-dlp`（字幕二级兜底，可选）
...
- **写笔记**：由 AI Agent 直接完成；`scripts/make_notes.py` 仅负责把逐字稿+模板组装成 `note_prompt.md` 草稿，不调用任何 LLM。
