#!/usr/bin/env python3
"""
make_notes.py — 【备选路径】用【任意 OpenAI 兼容的 LLM】把逐字稿整理成分类型 Markdown 笔记。

⚠️ 首选路径是让**主流 AI Agent**（Claude Code / Codex / WorkBuddy / Cursor 等）直接读
templates/ 下的模板 + 逐字稿写笔记——不依赖任何 LLM API，还能联网核实代码、读图表，更完整。
本脚本用于「没有可用 Agent」或「想离线批量生成」的场景：DeepSeek、通义千问(百炼兼容模式)、
Kimi、GPT、智谱 GLM 等，只要是 OpenAI 兼容接口都能用。
